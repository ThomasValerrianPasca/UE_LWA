...
//New Code
volatile int dropped_gringo = 0;

int itti_gtest(task_id_t destination_task_id)
{
  return lfds611_queue_check(itti_desc.tasks[destination_task_id].message_queue);
}

int itti_send_msg_to_task_try(task_id_t destination_task_id, instance_t instance, MessageDef *message)
{
  ...
  // check we have room in the queue, if not return -1
  if(lfds611_queue_check(itti_desc.tasks[destination_task_id].message_queue) < 100)
    return -1;
