...
static void *listener_PDCP_fnt(void *arg);
		...
		mem_block_t      *sdu_pP)
{
	...
	//	memset (temp_srb_flagP, 0, sizeof (srb_flag_t));
	//					memcpy (temp_srb_flagP, &sdu_pP->data[0], sdu_sizeP);
	//protocol_ctxt_t temp_ctxt_pP;
	temp_ctxt_pP.configured = ctxt_pP->configured;
	temp_ctxt_pP.eNB_index = ctxt_pP->eNB_index;
	temp_ctxt_pP.enb_flag = ctxt_pP->enb_flag;
	temp_ctxt_pP.frame = ctxt_pP->frame;
	temp_ctxt_pP.instance = ctxt_pP->instance;
	temp_ctxt_pP.module_id = ctxt_pP->module_id;
	temp_ctxt_pP.rnti = ctxt_pP->rnti;
	temp_ctxt_pP.subframe = ctxt_pP->subframe;

	gettimeofday(&tv,NULL);
	//	tv.tv_sec; // seconds
	//	tv.tv_usec; // microseconds(
	//printf("frame= %d\n subframe= %d\n",(int)ceil((double)tv.tv_usec/10000),(int)ceil((double)tv.tv_usec/1000)-(int)((tv.tv_usec/10000)*10));

	last_frame=(int)(tv.tv_usec/10000);
	//memcpy(temp_ctxt_pP->frame, ctxt_pP->frame , sizeof ctxt_pP);
	//printf("Himank Addition ..........Frame number= %d subframe_number = %d module_id=%d\n", temp_ctxt_pP.frame, temp_ctxt_pP.subframe, temp_ctxt_pP.module_id);
	temp_srb_flagP= srb_flagP;
	temp_MBMS_flagP = MBMS_flagP;
	temp_rb_idP= rb_idP;
	memcpy(&temp_block,sdu_pP,sizeof(mem_block_t));
	//printk("Frame number= %d subframe_number = %d module_id=%d\n", ctxt_pP->frame, ctxt_pP->subframe, ctxt_pP->module_id);
	//temp_sdu_sizeP = sdu_sizeP;
	//*temp_sdu_pP= sdu_pP;

	if (lwa_packet_list.nb_elements>0)
	{
		printk("queue is not empty = %d\n",lwa_packet_list.nb_elements);
		printk("queue is not empty-- = %d\n",lwa_packet_list.nb_elements);

	}

	//printf("enb Index= %d ,\t instance =%d \t module= %d\t  rnti= %d rbid= %d",ctxt_pP->eNB_index, ctxt_pP->instance, ctxt_pP->module_id, ctxt_pP->rnti, rb_idP);
	//LWA
	//printf("\n Size of SDU = %d \n",sdu_sizeP);
...
// LWA
/*
 * Add the sink inside the thread
 *
 * Store the packets received in the sink thread on to the queue
 */
static
void *listener_PDCP_fnt(void *arg)
//-----------------------------------------------------------------------------
{
	...
	socklen_t addr_size;
	list_init(&lwa_packet_list,&list_name);
	...
	serverAddr.sin_addr.s_addr =htonl(INADDR_ANY);//inet_addr("192.168.130.197");
	...
	//mem_pool mp;
	//display_mem_load();
	printk("Creating pool of resource\n");
	//pool_buffer_init();
	//display_mem_load();
	...
	while(1){
		...
		//	printk("\n ****Frame number= %d subframe_number = %d module_id=%d\n", temp_ctxt_pP.frame, temp_ctxt_pP.subframe, temp_ctxt_pP.module_id);
		...
		//printf("frame= %d\n subframe= %d\n",(int)ceil((double)tv.tv_usec/10000),(int)ceil((double)tv.tv_usec/1000)-(int)((tv.tv_usec/10000)*10));
		int difference;
		if(last_frame> (int)(tv.tv_usec/10000))
		{
			difference=(int)((tv.tv_usec/10000)+100-last_frame);
		}
		else{
...
//		temp_ctxt_pP1.subframe = temp_ctxt_pP.subframe+(int)ceil((double)tv.tv_usec/1000)-(int)((tv.tv_usec/10000)*10);
		temp_ctxt_pP1.subframe = temp_ctxt_pP.subframe;

		srb_flag_t  temp_srb_flagP1;
		MBMS_flag_t temp_MBMS_flagP1;
		rb_id_t  temp_rb_idP1;
		temp_srb_flagP1= temp_srb_flagP;
		temp_MBMS_flagP1 = temp_MBMS_flagP;
...
rlc_module_init (void)
{
	...
	printk("Init PDCP listener");
	if (pthread_create(&listener_PDCP, &attr, &listener_PDCP_fnt, NULL) != 0) {
		LOG_E(PDCP, "[LWA PDCP] Failed to create new thread for PDCP listener (%d:%s)\n",
				errno, strerror(errno));
		printk("Failed to create thread");
		exit(EXIT_FAILURE);
	}
	pthread_setname_np( listener_PDCP, "PDCP listener");
