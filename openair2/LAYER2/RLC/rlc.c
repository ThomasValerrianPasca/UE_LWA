...
srb_flag_t  temp_srb_flagP;
MBMS_flag_t temp_MBMS_flagP;
rb_id_t     temp_rb_idP;
sdu_size_t  temp_sdu_sizeP;
mem_block_t      *temp_sdu_pP;
...
//				mem_block_t *temp;
				...
				memcpy(&temp,sdu_pP->data,sdu_sizeP);
				sdu_pP->data=&temp_block;
...
//-----------------------------------------------------------------------------
{
	...
	while(1){
		...
		memset(temp_block,0,86);
		memcpy(&temp_block, &buffer, 86);

		flag_contains=1;
		//mem_block_t* temp_block;
		//temp_block=get_free_mem_block(86, __func__);
		//temp_block->pool_id=2;
//		if (temp_block != NULL) {
//			printf("temp block is not null");
//			memcpy(&temp_block->data, &buffer, 86);
//		}
		//printf ("Size of the list= %d",lwa_packet_list.nb_elements);
		//list_add_tail_eurecom(&temp_block,&lwa_packet_list);
		//printf ("Size of the list= %d",lwa_packet_list.nb_elements);
