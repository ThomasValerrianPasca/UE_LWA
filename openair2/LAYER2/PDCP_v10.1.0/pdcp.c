...
void Reordering_timeout(){

	while(true){
		//printf("thread_running =%d\n",thread_running);
		usleep(500);
		if(thread_running==true){
			//printf("Triggering thread\n");
			usleep(30000);
			//printf("PDPC timeout occurred\n");
			//while (pdcp_lock==1);

			//pdcp_lock=1;

			list_t      *sdu_list_p      = NULL;
			list_t      *sdu_reorder_list_p      = NULL;
			sdu_list_p = &pdcp_sdu_list;
			sdu_reorder_list_p= &pdcp_reorder_list;
//			printf("Number of elements in queue: %d\n",sdu_reorder_list_p->nb_elements);
			if(sdu_reorder_list_p->nb_elements >0 ){    // make sure that you are removing packet only if it previosly queued
				//send all the previously queued packet
				//		sdu_reorder_list_p
				//					printf("Number of elements in sdu list %d \n",sdu_reorder_list_p->nb_elements);
				//					if(sdu_reorder_list_p->nb_elements>0){
				//						mem_block_t* t;
				//						t=sdu_reorder_list_p->head;
				//						printf("In sequence print : ");
				//						while(t->next!=NULL){
				//							printf("%d \t ",t->identifier);
				//							t=t->next;
				//						}
				//						printf("%d \t ",t->identifier);
				//					}
				...
				//				printf("Packets sent from queue:");
				for(int i=sdu_reorder_list_p->nb_elements;i>0;i--)
				{

					mem_block_t *temp_sdu_p = list_get_from_reorder_list(sdu_reorder_list_p);
					list_add_tail_eurecom (temp_sdu_p, sdu_list_p);
					last_submitted_seq=temp_sdu_p->identifier;
					//	pdcp_p->last_submitted_pdcp_rx_sn = last_submitted_seq;
//					printf("\t poped from queue-> %d\n",temp_sdu_p->identifier);
...
//-----------------------------------------------------------------------------
{
	...
	if (pdcp_p->rlc_mode == RLC_MODE_UM && pdcp_p->last_submitted_pdcp_rx_sn != last_submitted_seq) {
			pdcp_p->last_submitted_pdcp_rx_sn = last_submitted_seq;
//			printf("last submitted sequence number = %d \n",last_submitted_seq);
		}
//while(pdcp_lock==1);
	pdcp_lock=1;
	...
	if (MBMS_flagP == 0 ) {
		if (srb_flagP) { //SRB1/2
			...
			//   printf("Sequence Number = %d",sequence_number);
		...
		//printf("Received  sequence number at PDPc =%d " ,sequence_number);
		if (pdcp_is_rx_seq_number_valid(sequence_number, pdcp_p, srb_flagP) == TRUE) {
			//    	printf("Himank In Sequence Delivery");
			oo_flag=false;
      ...
      LOG_D(PDCP, "Passing piggybacked SDU to RRC ...\n");*/
			...
			oo_flag=true;
//			printf("\tOut of Order\n");
			//pdcp_reorder_function(ctxt_pP, srb_flagP,  MBMS_flagP,  rb_idP,  sdu_buffer_sizeP, sdu_buffer_pP, sequence_number);

			//LOG_W(PDCP,	PROTOCOL_PDCP_CTXT_FMT"Incoming PDU has an unexpected sequence number (%d), RX window synchronisation have probably been lost!\n", PROTOCOL_PDCP_CTXT_ARGS(ctxt_pP, pdcp_p),sequence_number);
			//return TRUE;
			/*
	...
	if (FALSE == packet_forwarded ) {
		//  printf("Packet passed to higher layer \n with sdu_buffer_sizeP %d",sdu_buffer_sizeP);
...
#  endif
			...
			memcpy(&new_sdu_p->data[sizeof (pdcp_data_ind_header_t)], \
					...
					sdu_buffer_sizeP - payload_offset);
			new_sdu_p->identifier=sequence_number;
			gettimeofday(&tv,NULL);
			//	printf("Epoch time = %d oo_timer = %d , timer value= %d, time in seconds =%d \n",epoch_time,oo_timer, (int)((tv.tv_usec/1000)-oo_timer), (int)tv.tv_sec);
			//last_submitted_seq=sequence_number;
//						printf("%d  %d received sequence number = %d out of order flag=%d ",tv.tv_sec, tv.tv_usec,sequence_number, oo_flag);
			if (oo_flag==false || epoch_time>10)
			{

				//	printf("last oo flag %d \n",last_oo_flag);
				if(sdu_reorder_list_p->nb_elements >0 && peek_sequence_number_from_reorder_list(sdu_reorder_list_p)-sequence_number==1){    // make sure that you are removing packet only if it previously queued
					//send all the previously queued packet
					//		sdu_reorder_list_p
					//					printf("Number of elements in sdu list %d \n",sdu_reorder_list_p->nb_elements);
					//					if(sdu_reorder_list_p->nb_elements>0){
...
//						printf("Removed from queue");
						mem_block_t *temp_sdu_p = list_get_from_reorder_list(sdu_reorder_list_p);
						if (temp_sdu_p->identifier<sequence_number || temp_sdu_p->identifier> sequence_number+1)
						{
							list_add_tail_eurecom (temp_sdu_p, sdu_list_p);
							last_submitted_seq=temp_sdu_p->identifier;
//							printf("\t!-> %d",temp_sdu_p->identifier);
						}
						else{ //Current received packet has to be transmitted in order
							list_add_tail_eurecom (new_sdu_p, sdu_list_p);
//							printf("\t@ %d",new_sdu_p->identifier);
							last_submitted_seq=new_sdu_p->identifier;
							list_add_tail_eurecom (temp_sdu_p, sdu_list_p);
							last_submitted_seq=temp_sdu_p->identifier;
//							printf("\t@ %d",temp_sdu_p->identifier);
							if((temp_sdu_p->identifier - new_sdu_p->identifier)>1){
					...
					if(last_submitted_seq<sequence_number){
						list_add_tail_eurecom (new_sdu_p, sdu_list_p);
//						printf("\t# %d",new_sdu_p->identifier);
						last_submitted_seq=new_sdu_p->identifier;
					}
...
//					printf("\t inorder %d\n",new_sdu_p->identifier);
				...
				if (pdcp_p->rlc_mode == RLC_MODE_UM) {
					pdcp_p->last_submitted_pdcp_rx_sn = last_submitted_seq;
				}

			}
			else
			{ //If out of order packet is received hold the packet in reordering queue
				...
				{
					gettimeofday(&tv,NULL);
					oo_timer=(int)(tv.tv_usec/1000);
					oo_timer_sec=tv.tv_sec;

				}
				gettimeofday(&tv,NULL);
				if (oo_timer>(tv.tv_usec/1000)){
					epoch_time=epoch_time +(int)(oo_timer-(tv.tv_usec/1000));}
				else{
					epoch_time=epoch_time +(int)((tv.tv_usec/1000)-oo_timer);}
//				printf("\t added to queue %d\n",new_sdu_p->identifier);
				list_add_reorder (new_sdu_p, sdu_reorder_list_p);
				//Queue the packet in reordering buffer
			...
			LOG_D(PDCP, "Following content has been received from RLC (%d,%d)(PDCP header has already been removed):\n",
					...
					sdu_buffer_sizeP  - payload_offset);
			// printf("Following content has been received from RLC (%d,%d)(PDCP header has already been removed):\n",
			//             sdu_buffer_sizeP  - payload_offset + (int)sizeof(pdcp_data_ind_header_t),
			//             sdu_buffer_sizeP  - payload_offset);
	...
	/*if (oo_flag == 1 ){
	  //printf("Reciving Out of Order Packets");
...
//
////--------------------------------------------------------------------
//boolean_t pdcp_reorder_function( const protocol_ctxt_t* const ctxt_pP,
//		const srb_flag_t   srb_flagP,
//		const MBMS_flag_t  MBMS_flagP,
//		const rb_id_t      rb_idP,
//		const sdu_size_t   sdu_buffer_sizeP,
...
//
//
//	 printf("Packet is getting reordered %d",sdu_buffer_sizeP);
//	new_sdu_p = get_free_mem_block(sdu_buffer_sizeP - payload_offset + sizeof (pdcp_data_ind_header_t), __func__);
//
//	if (new_sdu_p) {
//		if (pdcp_p->rlc_mode == RLC_MODE_UM ) {
...
//#endif
//
//		memcpy(&new_sdu_p->data[sizeof (pdcp_data_ind_header_t)], &sdu_buffer_pP->data[payload_offset], sdu_buffer_sizeP - payload_offset);
//		list_add_tail_eurecom (new_sdu_p, sdu_list_p);
//	}
//	free_mem_block(sdu_buffer_pP, __func__);
//
...
//-----------------------------------------------------------------------------
{
	...
	list_init(&pdcp_reorder_list, NULL);
	...
	printk("Init Reordering timer");
	if (pthread_create(&PDCP_Reordering_timeout, &attr, &Reordering_timeout, NULL) != 0) {
		LOG_E(PDCP, "[LWA PDCP] Failed to create new thread for reordering timer (%d:%s)\n",
				errno, strerror(errno));
		printk("Failed to create thread");
		exit(EXIT_FAILURE);
	}
	thread_running=false;
	pthread_setname_np( PDCP_Reordering_timeout, "PDCP Reorder");
}
...
//-----------------------------------------------------------------------------
{
	...
	list_free (&pdcp_reorder_list);
	hashtable_destroy(pdcp_coll_p);
	pthread_cancel(PDCP_Reordering_timeout);
}
